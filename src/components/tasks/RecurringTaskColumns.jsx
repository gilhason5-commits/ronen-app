...
export default function RecurringTaskColumns() {
  ...
  // Persistent ordering state - loaded from user profile
  const [columnOrder, setColumnOrder] = useState(null);
  const [taskOrder, setTaskOrder] = useState({});

  // Load saved order from user profile on mount
  useEffect(() => {
  ...
  const orderedColumnKeys = useMemo(() => {
    ...
    // Keep only keys that still exist, append new ones
  ...
  const printColumn = useCallback((roleId) => {
    ...
    const rows = templateKeys.map(tmplId => {
      ...
      return `<tr>
        <td style="padding:6px 10px;border:1px solid #ddd;">${tmpl.taskTitle || '-'}</td>
        <td style="padding:6px 10px;border:1px solid #ddd;text-align:center;">${recurrence}</td>
      </tr>`;
    }).join('');

    printWindow.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8">
      <title>${col.name} - משימות</title>
      <style>
        body { font-family: Arial, sans-serif; padding: 20px; }
        h2 { margin-bottom: 4px; }
        .sub { color: #666; font-size: 13px; margin-bottom: 16px; }
        table { width: 100%; border-collapse: collapse; }
        th { background: #f5f5f4; padding: 8px 10px; border: 1px solid #ddd; text-align: right; }
      </style>
    </head><body>
      <h2>${col.name}</h2>
      ${col.employeeName ? `<div class="sub">${col.employeeName}</div>` : ''}
      <table>
        <thead><tr><th>משימה</th><th>תדירות</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </body></html>`);
                    ...
                    <div className="bg-stone-100 rounded-t-lg px-4 py-3 border border-b-0 border-stone-200">
                      <div className="flex items-center justify-between gap-1">
                        <button
                          ...
                          disabled={colIndex === orderedColumnKeys.length - 1}
                        ...
                        return (
                          <div key={tmplId} className="flex items-start gap-1">
                            <div className="flex flex-col gap-0.5 pt-2 flex-shrink-0">
                              <button
                                onClick={() => moveTask(roleId, taskIndex, -1)}
                                disabled={taskIndex === 0}
                                className="p-0.5 rounded hover:bg-stone-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                title="הזז למעלה"
                              >
                              ...
                              <button
                                onClick={() => moveTask(roleId, taskIndex, 1)}
                                disabled={taskIndex === templateKeys.length - 1}
                                className="p-0.5 rounded hover:bg-stone-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                title="הזז למטה"
                              >
