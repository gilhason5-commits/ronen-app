...
export default function AddRecurringTaskDialog({ open, onClose, editingAssignment = null }) {
  ...
  const [recurrenceType, setRecurrenceType] = useState(""); // "", once, weekly, monthly
  ...
  const { data: categories = [] } = useQuery({
    queryKey: ['taskCategories', 'RECURRING'],
    queryFn: async () => {
  ...
  // Roles that have exactly 1 active employee (for escalation)
  ...
  // Get the single employee for a role (for escalation)
  const getEmployeeForRole = (roleId) => {
    return activeEmployees.find(e => e.role_id === roleId) || null;
  };

  // Get employee by ID
  const getEmployeeById = (empId) => {
    return activeEmployees.find(e => e.id === empId) || null;
  };

  // Load editing assignment data
  useEffect(() => {
    if (editingAssignment && open && templates.length > 0) {
      // Find the employee and set the role
      const emp = allEmployees.find(e => e.id === editingAssignment.assigned_to_id);
      if (emp) {
        setSelectedEmployee(emp.id);
        if (emp.role_id) setSelectedRole(emp.role_id);
      }
  ...
  const handleTemplateSelect = (template) => {
    ...
    // Set default date to today and time based on schedule_rule if available
  ...
  const handleAddTask = () => {
    ...
    if (recurrenceType === 'once' && (!startDate || !startTime)) {
      toast.error('יש למלא תאריך ושעה');
      return;
    }

    if (recurrenceType === 'daily' && !startTime) {
      toast.error('יש למלא שעה');
      return;
    }

    if (recurrenceType === 'weekly' && (selectedWeekdays.length === 0 || !startTime)) {
      toast.error('יש לבחור לפחות יום אחד בשבוע ושעה');
      return;
    }

    if (recurrenceType === 'monthly' && !startTime) {
      toast.error('יש למלא שעה');
      return;
    }
    ...
    // If editing, update the existing assignment (single employee)
    if (editingAssignment) {
      ...
      const escalationFields = {
        escalation_role_id: updateData.escalation_role_id,
        escalation_role_name: updateData.escalation_role_name,
        escalation_employee_id: updateData.escalation_employee_id,
        escalation_employee_name: updateData.escalation_employee_name,
        escalation_employee_phone: updateData.escalation_employee_phone,
        escalate_to_manager: updateData.escalate_to_manager,
      };

      base44.entities.TaskAssignment.filter({
        task_template_id: editingAssignment.task_template_id,
        assigned_to_id: updateData.assigned_to_id,
        status: 'PENDING',
      }).then(siblingTasks => {
        const updates = siblingTasks
          .filter(t => t.id !== editingAssignment.id)
          .map(t => base44.entities.TaskAssignment.update(t.id, escalationFields));
        if (updates.length > 0) {
          Promise.all(updates).then(() => {
            console.log(`Updated escalation on ${updates.length} sibling tasks`);
            queryClient.invalidateQueries({ queryKey: ['taskAssignments'] });
          });
        }
      });
    ...
    // Create mode: generate assignments for ALL selected employees
    ...
    for (const employee of employees) {
      ...
      if (recurrenceType === 'once') {
        ...
        const endDateTime = new Date(startDateTime.getTime() + (selectedTemplate.duration_minutes || 60) * 60000);
        assignments.push({
          ...baseAssignment,
        ...
        for (let dayOffset = 0; dayOffset < 30; dayOffset++) {
          const taskDate = new Date(today.getFullYear(), today.getMonth(), today.getDate() + dayOffset);
          if (taskDate.getDay() === 5 || taskDate.getDay() === 6) continue;
          const [hours, minutes] = startTime.split(':').map(Number);
          const startDateTime = new Date(taskDate.getFullYear(), taskDate.getMonth(), taskDate.getDate(), hours, minutes, 0, 0);
          const endDateTime = new Date(startDateTime.getTime() + (selectedTemplate.duration_minutes || 60) * 60000);
          assignments.push({
            ...baseAssignment,
            computed_start_time: startDateTime.toISOString(),
        ...
        const today = new Date();
        const sundayOfCurrentWeek = new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay());
        for (let week = 0; week < 12; week++) {
          selectedWeekdays.forEach(weekday => {
            const daysToAdd = week * 7 + weekday;
            const taskDate = new Date(sundayOfCurrentWeek.getFullYear(), sundayOfCurrentWeek.getMonth(), sundayOfCurrentWeek.getDate() + daysToAdd);
            const [hours, minutes] = startTime.split(':').map(Number);
            const startDateTime = new Date(taskDate.getFullYear(), taskDate.getMonth(), taskDate.getDate(), hours, minutes, 0, 0);
            const endDateTime = new Date(startDateTime.getTime() + (selectedTemplate.duration_minutes || 60) * 60000);
            assignments.push({
        ...
        const today = new Date();
        const dayOfMonth = parseInt(selectedDayOfMonth);
        for (let month = 0; month < 6; month++) {
          const taskDate = new Date(today.getFullYear(), today.getMonth() + month, 1);
          taskDate.setDate(Math.min(dayOfMonth, new Date(taskDate.getFullYear(), taskDate.getMonth() + 1, 0).getDate()));
          const [hours, minutes] = startTime.split(':').map(Number);
          const startDateTime = new Date(taskDate.getFullYear(), taskDate.getMonth(), taskDate.getDate(), hours, minutes, 0, 0);
          const endDateTime = new Date(startDateTime.getTime() + (selectedTemplate.duration_minutes || 60) * 60000);
          assignments.push({
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editingAssignment ? 'עריכת משימה' : 'הוספת משימה שוטפת מהמאגר'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          {(selectedTemplate || editingAssignment) && (
            <div className="space-y-4 border-t pt-4">
              ...
              {/* removed time edit restriction notice */}
              
              <div>
                <label className="text-sm font-medium text-stone-700 mb-2 block">
                  <Briefcase className="w-4 h-4 inline mr-1" />
                  {editingAssignment ? 'עובד' : 'עובדים'}
                </label>
                {editingAssignment ? (
              ...
              {recurrenceType === 'weekly' && (
                <>
                  <div>
                    <label className="text-sm font-medium text-stone-700 mb-2 block">ימים בשבוע (ראשון-חמישי בלבד)</label>
                    <div className="flex flex-wrap gap-2">
                      {weekdays.map(day => (
                        <Button
                          key={day.value}
                          ...
                          className={selectedWeekdays.includes(day.value) ? "bg-emerald-600 hover:bg-emerald-700" : ""}
                        >
                          {day.label}
                        </Button>
                      ))}
              ...
              {recurrenceType === 'monthly' && (
                <>
                  <div>
                    <label className="text-sm font-medium text-stone-700 mb-2 block">יום בחודש</label>
              ...
              <div className="border-t pt-4">
                <label className="text-sm font-medium text-stone-700 mb-2 block">
                  מנהל אסקלציה (אופציונלי)
                </label>
                <p className="text-xs text-stone-500 mb-2">
                  אם נבחר - בעל התפקיד יקבל הודעה כשהמשימה לא בוצעה או באיחור
                </p>
                <Select value={escalationRole} onValueChange={setEscalationRole}>
                  <SelectTrigger>
                    <SelectValue placeholder="ללא מנהל אסקלציה" />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    {rolesWithOneEmployee.map(role => {
                      ...
                      return (
                        <SelectItem key={role.id} value={role.id}>
                          {role.role_name} ({emp?.full_name}) - {role.department_name}
                        </SelectItem>
                      );
                    })}
                  </SelectContent>
                </Select>
                {escalationRole && escalationRole !== 'none' && (
                  <p className="text-xs text-stone-500 mt-1">
                    יקבל הודעה: {getEmployeeForRole(escalationRole)?.full_name || ''}
                  </p>
                )}
              </div>
