...
// Pattern copied from tasksScheduler.js for consistency.
...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`🕐 DailyAvailabilityScheduler started at ${currentTime.toISOString()}`);
    ...
    // Parse target date from payload or default to today
    let targetDate;
    try {
    ...
    if (!targetDate) {
      ...
      targetDate = new Date(currentTime.toLocaleString('en-US', { timeZone: 'Asia/Jerusalem' }));
    ...
    console.error("❌ dailyAvailabilityScheduler error:", error.message);
...
async function sendWhatsappTemplate(accountSid, authToken, fromNumber, toPhone, contentSid, templateData) {
  try {
    ...
    const contentVariables = JSON.stringify({
      ...
      "3": templateData.event_date,
      "4": templateData.record_id
    });
    ...
    if (contentSid) {
      ...
      formData.append("Body", `שלום ${templateData.employee_name},\nהאם את/ה מגיע/ה לאירוע "${templateData.event_name}" בתאריך ${templateData.event_date}?\nהשב: ✅ מגיע / ❌ לא מגיע`);
    }
